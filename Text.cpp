...
void Text::read_text(){
    ...
    #pragma omp parallel for private(it) shared(Text::listOfWordsFromText)
    {
        for (it = list_of_words_from_text.begin(); it != list_of_words_from_text.end(); it++) {
            std::cout << *it << "Watek: " << omp_get_thread_num() << std::endl;
        }
    }
